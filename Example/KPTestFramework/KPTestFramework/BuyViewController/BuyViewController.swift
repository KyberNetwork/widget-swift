...
extension BuyViewController: QRCodeReaderDelegate {
...
extension BuyViewController: KWCoordinatorDelegate {
  ...
  func coordinatorDidBroadcastTransaction(with hash: String) {
    self.coordinator?.stop(completion: {
      self.showAlertController(title: "Buy transaction sent", message: "Tx hash: \(hash)")
      self.coordinator = nil
    })
  }
}
